...
| 社交/低可信来源 <-> UI | 事实状态和 `trust` 只用于解释；是否可发布由配置时准入和结构质量门决定 |
...
- 非中文内容在没有翻译 API key 或翻译失败时保留原文，标记为可见降级并等待后续补译。
...
RLS/execute grant、refresh/cron 鉴权、错误脱敏和并发租约必须持续由迁移与测试保护。只读 CORS 与限流策略可继续独立收紧，但不能削弱 server-side 安全边界。
...
如果未来添加账号、行为分析、集中日志或新的数据处理方，需要新建或更新隐私说明。
...
| 不可信新闻误导用户 | 配置时来源准入、结构质量门、事实状态、证据链和 `trust` 解释字段 | 这些规则是启发式，不等于事实核验 |
...
- 不提供法律合规审计。
- 不提供事实核查保证。
- 不实现账号权限、审计日志、WAF、独立监控平台或托管式密钥管理；现有 Vercel CDN 缓存与 Supabase Vault 不因此失效。
- 不绕过新闻网站访问控制。
...
- 涉及 API 的改动先区分公开只读与受保护写入口，并同步更新鉴权、缓存、限流和错误脱敏文档。
