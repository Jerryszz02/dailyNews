...
| `src/lib/sourceCoverage.ts` | 生产每轮计划全部启用且已准入来源；有限并发负责资源保护，不再用跨时槽轮转控制覆盖 |
...
| 全来源刷新 | 生产每 2 小时运行一轮；全部启用来源在同一轮进入计划，来源数量不再线性扩大新闻的计划等待时间 |
| 可解释精选 | 精选事件保存面向读者的理由，以及公共影响、时效、证据与事实状态依据 |
| 独立热点 | 热点按来源角色、独立证据、传播增速和 24 小时半衰期计算，不与公共重要性合成一个分数 |
| 全分类日报 | 每天上海时间 08:00 形成前 24 小时日报，覆盖当前十个栏目中实际有合格事件的栏目；同一 edition 首次成功发布后保持不变 |
...
| PRD-13 | 每次生产刷新必须把全部 `enabled && approved` 来源加入计划；并发上限只能控制同时执行数，不能把来源推迟到下个时槽 | refresh/source coverage 测试断言计划数等于完整来源数 |
| PRD-14 | 精选事件必须提供可读选择理由和结构化依据；偏好仍不能改变今日必知 | curation 与前端卡片测试通过 |
| PRD-15 | 热点只表达事件在 48 小时窗口内的证据传播强度，采用 24 小时半衰期，并标注新出现/上升/突增/持续及时间置信度 | heat fixture 与 compact round-trip 测试通过 |
| PRD-16 | 每天上海时间 08:00 生成前 24 小时全分类日报；每个事件最多出现一次，分类无合格事件时允许为空 | daily edition cutoff、分类与引用不变量测试通过 |
| PRD-17 | LLM 去重默认关闭；开启后只判断确定性规则的灰区新候选，每候选最多 3 个近邻、每轮默认最多 12 次调用，失败时不合并且不阻断发布 | semantic dedupe 限额、失败与置信度测试通过 |
...
- 页面能打开并展示全分类日报、正在升温、全部最新、今日必知、重要进展、持续关注、分类事件、搜索和偏好设置。
- 有效候选到 stories、24 小时 stories 到 latest 的映射率均为 100%，`unmappedCandidateCount=0`。
- 2 小时调度下，每轮计划全部启用且已准入来源，单一来源失败不影响其它来源进入本轮 worker。
...
| 长期内容 SLA | 当前为 2 小时全来源调度；是否提高刷新频率需结合完整 sweep 的生产耗时、外部限流和成本数据 |
