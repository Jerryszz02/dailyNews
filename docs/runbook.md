...
The deterministic monitor was calibrated for the former five-minute cadence and rolling 30-minute source-coverage gate. Do not start a new formal burn-in after the two-hour cost-control change until its slot and coverage rules are redesigned; preserve existing output only as historical evidence.
...
The expected deployment is immutable within one output directory. If the production alias moves, the monitor records `deployment_alias_mismatch`, changes to `needs_review`, and exits. Stop the old LaunchAgent, preserve its evidence directory, then start the new deployment with a new output directory; never delete or reuse the old state to splice two deployments into one acceptance window.
