...
`stop` only terminates the local observer and unloads its LaunchAgent. It preserves the output directory and does not disable the independent Supabase Cron that calls `/api/cron` every 5 minutes. Pause any separate Codex heartbeat independently if one was created for status reminders.
