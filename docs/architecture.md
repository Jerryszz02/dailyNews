...
2. `scripts/newsService.ts` runs one continuously replenished pool of up to 50 source tasks within a 240-second collection budget. Each task prefers its configured RSS/Atom feed or news sitemap, falls back to its public page, and uses Firecrawl keyless only when direct collection has no usable recent candidates. Feed endpoints have explicit host scope separate from article admission; reviewed `readerUrlAliases` declare additional publisher reader hosts (such as BBC UK and Mtime content), never inferred from feed links; redirects and candidate URLs remain checked. Network/provider work and source attempts have independent bounds. Transport connection failures receive at most one retry under the same source deadline; HTTP denials and policy failures do not. Opt-in `collectNewsCandidates({ includeDiagnostics: true })` separates direct/fallback status, duration and logical search call count from enrichment time and remaining pending translations. SDK search calls are not counted by a global `fetch` wrapper.
   `scripts/xSource.ts` reads official X user timelines with a server-only bearer token. User IDs, incremental IDs and incomplete pagination state are persisted with source results and candidates in the same fenced transaction. No token means X is excluded from the active registry; configured website collection continues. Replies and retweets are excluded. X and Firecrawl use four-request provider gates during acquisition. After all source acquisition tasks finish, a separate four-worker enrichment pass translates and repairs the collected items within the remaining global budget. Translation never occupies a source acquisition slot or its 45-second deadline. A failed direct attempt gives Firecrawl a fresh bounded source attempt within the same global deadline.
3. Candidate identity and original text survive enrichment independently and are persisted in the same atomic terminal commit. A transient translation failure receives one final retry after the first enrichment pass; any remaining failures keep the original text visible with `translationStatus: pending`; missing summaries and publication times degrade to explicit pending/estimated states instead of deleting the candidate.
4. `src/lib/curation.ts` rejects only unapproved/out-of-domain sources, invalid or missing identity, navigation pages and explicit promotion. Discovery and publication share `articleIdentity.ts` so stored script-template links and directory/language-switch pages are removed on republication as well. Legacy HTTP links are requested only through their HTTPS equivalent, subject to the same admission and network guards. `trust.shouldShow` does not affect inclusion, tier or ranking; `trust.level` can establish a confirmed fact status, and confirmed stories are excluded from the watchlist layer while remaining available in stories, latest and category views.
