...
2. `scripts/newsService.ts` gives Firecrawl keyless at most 8 seconds, then uses bounded-concurrency direct page/feed fetches. The collection phase has a 22-second default deadline so cold start, persistence and publication can remain inside the 30-second refresh target; it emits live candidates plus per-source outcomes and does not insert fallback content into the production candidate path.
...
6. `scripts/newsRefresh.ts` acquires a fenced Supabase lease, persists source outcomes and candidates, reads the rolling 72-hour pool, applies absolute/relative quality gates and publishes only content changes. Quiet success updates durable check time without changing `reportId/generatedAt`.
7. `scripts/supabaseNewsStore.ts` accesses private tables only through service-role RPC. Snapshot insert, run completion and latest pointer switch are atomic; stale workers cannot publish with an old fencing token.
