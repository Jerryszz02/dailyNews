...
2. `scripts/newsService.ts` runs Firecrawl keyless web/news search and direct page/feed/sitemap collection concurrently within a 45-second budget. The default 24-source pool bounds resource use, each source attempt has an independent timeout, sitemap limits are honored, and both requested and redirect-final URLs must remain inside the source allowlist.
