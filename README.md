...
Daily News 是一个事件级新闻日报：从已准入来源发现候选，把不同媒体对同一事件的报道合并为证据链，并同时提供全部最新、可解释精选、正在升温和每天 08:00 的全分类日报。
...
LLM 事件去重默认关闭。只有同时配置翻译 API key 和 `DAILY_NEWS_LLM_DEDUPE_ENABLED=true` 时，系统才把确定性规则无法判定的新候选交给同一 OpenAI-compatible 模型；每条候选最多比较 3 个近邻，每轮默认最多 12 次调用，只在 `same_event` 且置信度至少 0.8 时合并。模型失败时保守地不合并，不阻塞发布。
...
`GET /api/news` 默认返回完整 `DailyNewsReport` V2、兼容 `items`，并增加 `latestStories`、`hotStories`、`dailyEdition` 与刷新元数据；旧 V2 仍可读取。普通读取使用 30 秒共享缓存；仅紧凑视图允许 `view=web&reload=1`，该响应使用 `no-store`，并受短时服务端读取合并与每客户端限流保护。`servingMode`、`pipelineStatus`、`contentStatus` 分别表达服务来源、流水线健康和内容新鲜度；存在有效 last-known-good 时 `/api/news` 与 `/api/health` 均返回 200 并如实标记 degraded/stale，完全没有可服务报告时才返回 503。
...
- `src/lib/dedupe.ts`：确定性同一事件聚类去重，并接收已持久化的语义事件锚点。
- `src/lib/curation.ts`：候选有效性、事件证据、事实状态、公共影响分层、精选理由、热点与全分类日报。
- `src/lib/sourceCoverage.ts`：生产全来源计划和兼容的受限手动选择逻辑。
...
- `scripts/semanticDedupe.ts`：只处理灰区候选的限额 LLM 事件关系判断。
